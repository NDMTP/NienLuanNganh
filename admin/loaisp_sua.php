              ...
              <div class="col-6 col-md-6 col-lg-6">
                <div class="card">
                  <form method="GET" action="loaisp_crud.php">
                    ...
                    <?php
                    // Lấy thông tin của loại sản phẩm cần sửa dựa trên mã loại (được truyền qua URL)
                    $maloai = $_GET['maloai']; // lấy mã loại từ URL
                    $sql = "SELECT * FROM loaisanpham WHERE MALOAI = '$maloai'";
                    $result = $conn->query($sql);

                    // Kiểm tra nếu tồn tại loại sản phẩm
                    if ($result->num_rows > 0) {
                      $row = $result->fetch_assoc();
                    } else {
                      echo "<script>alert('Loại sản phẩm không tồn tại!'); history.back();</script>";
                      exit();
                    }
                    ?>
                    <div class="card-body">
                      <div class="mb-3">
                        <label class="form-label" for="basic-icon-default-fullname">Mã loại</label>
                        <div class="input-group input-group-merge">
                          <span id="basic-icon-default-fullname2" class="input-group-text"></span>
                          <input type="text" value="<?php echo $row['MALOAI']; ?>" name="maloai" class="form-control"
                            id="basic-icon-default-fullname" readonly />
                        </div>
                      </div>
                      <div class="mb-3">
                        <label class="form-label" for="basic-icon-default-company">Tên loại</label>
                        <div class="input-group input-group-merge">
                          <span id="basic-icon-default-fullname2" class="input-group-text"></span>
